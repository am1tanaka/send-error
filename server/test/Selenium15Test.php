...
/** Seleniumの継承*/
class Selenium15Test extends \PHPUnit_Extensions_Selenium2TestCase
{
    ...
    public function testEntryErrorDescOver()
    {
        // エラー数を確認
        ...
        $res = $this->entryError($mes);
        $this->assertRegExp('/message size over/', $res['response']);

        // エラーが増えていないことを確認
        $this->assertEquals($errnum, ErrorTable::all()->count(), 'check same error num.');
        ...
        $res = $this->entryError($mes);
        $this->assertRegExp('/entry ok/', $res['response']);

        // エラーが増えていないことを確認
        $this->assertEquals($errnum+1, ErrorTable::all()->count(), 'check error entry.');
    }
}
